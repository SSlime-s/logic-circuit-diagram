...
export function MakeTable(parts: Parts) {
  ...
  while (dfs.length) {
    ...
    parts.dots[idx].line.forEach(v => {
      const nxt =
        parts.lines[v][0] !== idx ? parts.lines[v][0] : parts.lines[v][1];
      if (dotsFormula[nxt]) {
        cnt++;
      } else {
    ...
    parts.dots[idx].and.forEach(v => {
      ...
      if (dotsFormula[nxt]) {
        ...
        dotsBool.forEach((_, index) => {
          dotsBool[index][nxt] =
            dotsBool[index][parts.ands[v][0]] &&
            dotsBool[index][parts.ands[v][1]];
        });
        dfs.push({
          idx: nxt,
          formula: dotsFormula[nxt]
    ...
    parts.dots[idx].or.forEach(v => {
      ...
      if (dotsFormula[nxt]) {
        ...
        dotsBool.forEach((_, index) => {
          dotsBool[index][nxt] =
            dotsBool[index][parts.ors[v][0]] ||
            dotsBool[index][parts.ors[v][1]];
        });
        dfs.push({
          idx: nxt,
          formula: dotsFormula[nxt]
